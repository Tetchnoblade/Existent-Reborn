    ...
    public void renderPlayerlist(int width, Scoreboard scoreboardIn, @Nullable ScoreObjective scoreObjectiveIn) {
        ...
        for (int k4 = 0; k4 < l3; ++k4) {
            ...
            if (k4 < list.size()) {
                ...
                if (Existent.getModuleManager().isEnabled(StreamerMode.class)) {
                    if (StreamerMode.scrambleNames && !s4.contains(mc.thePlayer.getName())) {
                        s4 = "\247aProtected";
                    }
                    if (StreamerMode.nameprotect) {
                        s4 = s4.replace(this.mc.thePlayer.getName(), Existent.getClientPrefix(StreamerMode.name));
                    }
                }
