package zyx.existent.utils.render.shader;
...
public abstract class Shader implements MCUtil {
    private int program;
    ...
    public Shader(final String fragmentShader) {
        ...
        try {
            final InputStream vertexStream = mc.getResourceManager().getResource(new ResourceLocation("existent/shader/vertex.vert")).getInputStream();
            vertexShaderID = createShader(IOUtils.toString(vertexStream), ARBVertexShader.GL_VERTEX_SHADER_ARB);
            IOUtils.closeQuietly(vertexStream);

            final InputStream fragmentStream = mc.getResourceManager().getResource(new ResourceLocation("existent/shader/fragment/" + fragmentShader)).getInputStream();
