...
public class GuiAnimationSettingsOF extends GuiScreen {
    ...
    public void initGui() {
        ...
        for (int i = 0; i < enumOptions.length; ++i) {
            ...
            if (!gamesettings$options.getEnumFloat()) {
                this.buttonList.add(new GuiExOptionButtonOF(gamesettings$options.returnEnumOrdinal(), j, k, gamesettings$options, this.settings.getKeyBinding(gamesettings$options)));
            } else {
                this.buttonList.add(new GuiExOptionSliderOF(gamesettings$options.returnEnumOrdinal(), j, k, gamesettings$options));
            }
        }

        this.buttonList.add(new GuiExButton(210, this.width / 2 - 155, this.height / 6 + 168 + 11, 70, 20, Lang.get("of.options.animation.allOn")));
        this.buttonList.add(new GuiExButton(211, this.width / 2 - 155 + 80, this.height / 6 + 168 + 11, 70, 20, Lang.get("of.options.animation.allOff")));
        this.buttonList.add(new GuiExOptionButton(200, this.width / 2 + 5, this.height / 6 + 168 + 11, I18n.format("gui.done")));
    }
    ...
    protected void actionPerformed(GuiButton button) {
        if (button.enabled) {
            if (button.id < 200 && button instanceof GuiExOptionButton) {
                this.settings.setOptionValue(((GuiExOptionButton) button).returnEnumOptions(), 1);
